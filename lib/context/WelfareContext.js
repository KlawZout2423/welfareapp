...
const stripePromise = loadStripe(process.env.NEXT_PUBLIC_STRIPE_PUBLIC_KEY || 'pk_test_123');
...
export function WelfareProvider({ children }) {
  ...
  // Safe JSON extractor to prevent "Unexpected token '<' is not valid JSON" errors
  ...
  useEffect(() => {
    ...
    if (paymentSuccess && sessionId && type && metadataStr) {
        ...
        try {
          ...
          if (type === 'dues') {
            ...
            if (!res.ok) {
              ...
              showToastMsg(`Installment of GH₵${metadata.paymentAmount}.00 settled and verified successfully!`);
              fetchPortalData();
            }
          }
        } catch (err) {
  ...
  const handleSendSMS = async (e) => {
    ...
    if (!smsData.message || !smsData.message.trim()) {
      showToastMsg("Message content cannot be empty.", "error");
      return;
    }
    ...
    try {
      const res = await fetch("/api/sms-dispatch", {
        ...
        body: JSON.stringify({
          ...
          type: smsData.type,
          targetMemberId: smsData.targetMemberId || "",
          customPhones: smsData.customPhones || "",
        })
      });
      const data = await res.json();
      if (!res.ok || !data.success) {
        showToastMsg(data.error || "Failed to send SMS.", "error");
        return;
      }
      setSmsData(prev => ({ ...prev, message: "" }));
      showToastMsg(data.message || "SMS sent successfully!");
      fetchPortalData();
    } catch (err) {
      showToastMsg("Error sending SMS. Please try again.", "error");
    } finally {
  ...
  const handleStaffContributeDues = async (selectedMonth) => {
    const month = selectedMonth || "July 2026";
    const amount = schemeConfig.monthlyContribution;

    try {
      showToastMsg("Initializing secure checkout...", "info");
      const res = await fetch("/api/portal", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      showToastMsg(err.message || "Checkout could not be initialized.", "error");
    }
  };
