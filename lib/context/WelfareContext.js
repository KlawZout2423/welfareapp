...
export function WelfareProvider({ children }) {
  ...
  const handleSendSMS = async (e) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok || !data.success) {
        showToastMsg(data.error || "Failed to send SMS broadcast.", "error");
        return;
      }
      setSmsData(prev => ({ ...prev, message: "" }));
      showToastMsg(data.message || "SMS Broadcast sent successfully!");
      fetchPortalData();
    } catch (err) {
      showToastMsg("Error sending broadcast. Please try again.", "error");
    } finally {
