...
export function WelfareProvider({ children }) {
  ...
  const handleStaffContributeDues = async (selectedMonth) => {
    ...
    const mockRef = `demo-TX-${Math.random().toString(36).substr(2, 9).toUpperCase()}`;

    // Instant payment processing for local demo
    try {
      const res = await fetch("/api/portal", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (!res.ok) {
        ...
        setShowPaymentModal(false);
        showToastMsg(`Welfare dues for ${month} paid and verified successfully!`);
        fetchPortalData();
      }
    } catch (err) {
